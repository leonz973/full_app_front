            ...
            <view class="uni-list-cell cell-pd">
                <view class="uni-uploader">
                    <view class="uni-uploader-head" v-if="!readOnly">
                        <view class="uni-uploader-title" v-if="type === 'img'"
                            >添加图片</view
                        >
                        <view class="uni-uploader-title" v-if="type === 'video'"
                            >点击播放选好的视频</view
                        >
                        <view class="uni-uploader-info"
                            >{{ FJXX.length }}/{{ count }}</view
                        >
                    </view>
                    <view class="uni-uploader-body">
                        <view class="uni-uploader__files">
                            <block v-for="(item, i) in FJXX" :key="i">
                                ...
                                <view
                                    class="uni-uploader__file uploader_video"
...
export default {
    ...
    methods: {
            ...
            uni.chooseImage({
                ...
                success: (res) => {
                    ...
                    FilePaths.forEach((item, i) => {
                        // $that.FJXX.push({
                        //     fileType: $that.fileTypeTest(item),
                        // 	filePath: item
                        // })
                            ...
                            if (data.WJID) {
                                try {
                                    ...
                                    this.FJXX.push({
                                        ...
                                        WDLJ: fjitem.MLSY,
                                        filePath: item,
                                        fileType: this.fileTypeTest(fjitem.WJMC) //获取文件后缀名称
                                    });
                                } catch (e) {
                                    this.$pdAction.toast(
                                        '附件信息数据转换失败'
        ...
        //把数据上传到后台
        uploadFile(Path) {
            let url = this.uploadServer
                ? this.uploadServer
                : this.$pdGlobal.UPLOADSERVICE;
        ...
        downloadFile(list) {
            var _this = this;
            if (list.length > 0) {
                this.$pdAction.showWait(
                    '正在下载截图1' + '/' + list.length + ''
                );
                list.forEach((item, i) => {
                    this.download(item.WJBH, Number(i + 1), list.length).then(
                        (res) => {
                            var obj = {
        ...
        download(WJBH, i, len) {
            var url =
                this.$pdGlobal.SERVICEURL +
                '/smallroutine/smallroutinecontroller/downloadfilebyid/' +
                WJBH;
            return new Promise((resolve, reject) => {
                uni.downloadFile({
                    url: url,
    ...
    watch: {
        uploadNum(num) {
            this.$pdAction.showWait(
                '正在下载截图' + num + '/' + this.fileArray.length + ''
            );
