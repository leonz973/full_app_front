...
export default {
    ...
    methods: {
        ...
        setContent(html, append) {
            ...
            if (!html) {
                if (this.rtf && !append)
                    this.rtf.parentNode.removeChild(this.rtf);
                return;
            }
            ...
            for (
                var styles = this.rtf.getElementsByTagName('style'),
                    i = 0,
                    style;
                (style = styles[i++]);
            ...
            for (let i = 0, j = 0, img; (img = imgs[i]); i++) {
                if (
                    parseInt(img.style.width || img.getAttribute('width')) >
                    windowWidth
                )
                    img.style.height = 'auto';
                fill(img);
                if (
                    !img.hasAttribute('ignore') &&
