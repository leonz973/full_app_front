 ...
 * @property {String} active-bg-color 起始/结束日期按钮的背景色(默认#3caaff)
 * @property {String Number} z-index 弹出时的z-index值(默认10075)
 * @property {String} active-color 起始/结束日期按钮的字体颜色(默认#ffffff)
 * @property {String} range-bg-color 起始/结束日期之间的区域的背景颜色(默认rgba(41,121,255,0.13))
 * @property {String} range-color 选择范围内字体颜色(默认#3caaff)
    ...
    methods: {
        ...
        btnFix(show) {
            ...
            if (this.mode == 'date') {
                ...
                let result = `${year}-${this.formatNum(month)}-${this.formatNum(
                    day
                )}`;
