...
<template>
    <view class="p-mui-tree">
        <template v-if="showLoading">
            <view class="p-mui-tree-loader p-mui-tree-flex-center">
                <view class="p-mui-tree-loader-inner">加载中...</view>
            </view>
        </template>

        <template v-else>
            <view v-if="isEmpty || !visible" class="p-mui-tree-empty">
                {{ emptyText }}
            </view>
            <view
                :key="updateKey"
                class="p-mui-tree-tree"
...
<script>
/* 此组件来自于p-mui-tree-tree https://ext.dcloud.net.cn/plugin?id=1000#rating */
// import Vue from 'vue'
import TreeStore from './model/tree-store.js';
import { getNodeKey } from './tool/util.js';
// import TreeNode from './p-mui-tree-node.vue';
...
export default {
    ...
    computed: {
        isEmpty() {
            if (this.store.root) {
                const childNodes = this.store.root.getChildNodes(
                    this.childNodesId
                );
    ...
    watch: {
        ...
        childNodesId() {
            this.$nextTick(() => {
                this.$emit('p-mui-tree-tree-render-completed');
            });
        },
        treeData: {
            handler(newVal) {
...
.p-mui-tree-loader-inner,
.p-mui-tree-loader-inner:before,
.p-mui-tree-loader-inner:after {
    background: #efefef;
