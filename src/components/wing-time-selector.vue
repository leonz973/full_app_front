...
export default {
    ...
    props: {
        hideMins: {
            //这里没有年到小时， 所以在年到分时 如果hideMins为true 则隐藏分钟的选择
            type: Boolean,
            default: false
        },
        showType: {
    ...
    created() {
        ...
        if (
            this.getMillisecond(this.beginDate) >
            this.getMillisecond(this.endDate)
        ) {
        ...
        this.beginMonth =
            beginDateList[1].charAt(0) == '0'
                ? beginDateList[1].charAt(1)
                : beginDateList[1];
        this.beginDay =
            beginDateList[2].charAt(0) == '0'
                ? beginDateList[2].charAt(1)
                : beginDateList[2];
        let endDateList = this.endDate.split('-');
        this.endYear = endDateList[0];
        this.endMonth =
            endDateList[1].charAt(0) == '0'
                ? endDateList[1].charAt(1)
                : endDateList[1];
        this.endDay =
            endDateList[2].charAt(0) == '0'
                ? endDateList[2].charAt(1)
                : endDateList[2];
        // 时间
        let beginTimeList = this.beginTime.split(':');
        this.beginHour =
            beginTimeList[0].charAt(0) == '0'
                ? beginTimeList[0].charAt(1)
                : beginTimeList[0];
        this.beginMinute =
            beginTimeList[1].charAt(0) == '0'
                ? beginTimeList[1].charAt(1)
                : beginTimeList[1];
        this.beginSecond =
            beginTimeList[2].charAt(0) == '0'
                ? beginTimeList[2].charAt(1)
                : beginTimeList[2];
        let endTimeList = this.endTime.split(':');
        this.endHour =
            endTimeList[0].charAt(0) == '0'
                ? endTimeList[0].charAt(1)
                : endTimeList[0];
        this.endMinute =
            endTimeList[1].charAt(0) == '0'
                ? endTimeList[1].charAt(1)
                : endTimeList[1];
        this.endSecond =
            endTimeList[2].charAt(0) == '0'
                ? endTimeList[2].charAt(1)
                : endTimeList[2];
        this.getDate();
        for (
            var i = parseInt(this.beginYear);
            i <= parseInt(this.endYear);
            i++
        ) {
            this.years.push(i + '年');
        }
        ...
        this.getDays();
        for (
            var i = parseInt(this.beginHour);
            i <= parseInt(this.endHour);
            i++
        ) {
            this.hours.push(i + '时');
        }
        if (this.beginHour == this.endHour) {
            if (parseInt(this.beginMinute) > parseInt(this.endMinute)) {
                for (
                    var i = parseInt(this.endMinute);
                    i <= parseInt(this.beginMinute);
                    i++
                ) {
                ...
                for (
                    var i = parseInt(this.beginMinute);
                    i <= parseInt(this.endMinute);
                    i++
                ) {
                    this.minutes.push(i + '分');
                }
        ...
        if (this.beginMinute == this.endMinute) {
            if (parseInt(this.beginSecond) > parseInt(this.endSecond)) {
                for (
                    var i = parseInt(this.endSecond);
                    i <= parseInt(this.beginSecond);
                    i++
                ) {
                ...
                for (
                    var i = parseInt(this.beginSecond);
                    i <= parseInt(this.endSecond);
                    i++
                ) {
                    this.seconds.push(i + '秒');
                }
    ...
    methods: {
                ...
                case 'time':
                    {
                        if (column == 0) {
                            ...
                            if (this.minutes.length == 1) {
                                if (
                                    parseInt(this.beginSecond) >
                                    parseInt(this.endSecond)
                                ) {
                                    for (
                                        var i = parseInt(this.endSecond);
                                        i <= parseInt(this.beginSecond);
                ...
                case 'monthToDay':
                    {
                        this.getMonths();
                        if (column == 0) {
                            this.month = this.months[value].substring(
                                0,
                                this.months[value].length - 1
                            );
                            this.getDays();
                        }
        ...
        confirm(e) {
            ...
            switch (this.showMode) {
                ...
                case 'month':
                    {
                        date = {
                            key: this.months[value[0]].substring(
                                0,
                                this.months[value[0]].length - 1
                            ),
                ...
                case 'day':
                    {
                        date = {
                            key: this.days[value[0]].substring(
                                0,
                                this.days[value[0]].length - 1
                            ),
                ...
                case 'hour':
                    {
                        date = {
                            key: this.hours[value[0]].substring(
                                0,
                                this.hours[value[0]].length - 1
                            ),
                ...
                case 'minute':
                    {
                        date = {
                            key: this.minutes[value[0]].substring(
                                0,
                                this.minutes[value[0]].length - 1
                            ),
                ...
                case 'second':
                    {
                        date = {
                            key: this.seconds[value[0]].substring(
                                0,
                                this.seconds[value[0]].length - 1
                            ),
                ...
                case 'yearToMonth':
                    {
                        let month = this.months[value[1]].substring(
                            0,
                            this.months[value[1]].length - 1
                        );
                        date = {
                ...
                case 'monthToDay':
                    {
                        let month = this.months[value[0]].substring(
                            0,
                            this.months[value[0]].length - 1
                        );
                        let day = this.days[value[1]].substring(
                            0,
                            this.days[value[1]].length - 1
                        );
                        date = {
                            month,
                            day,
                ...
                case 'hourToMinute':
                    {
                        let hour = this.hours[value[0]].substring(
                            0,
                            this.hours[value[0]].length - 1
                        );
                        let minute = this.minutes[value[1]].substring(
                            0,
                            this.minutes[value[1]].length - 1
                        );
                        date = {
                            hour,
                            minute,
                ...
                case 'minuteToSecond':
                    {
                        let minute = this.minutes[value[0]].substring(
                            0,
                            this.minutes[value[0]].length - 1
                        );
                        let second = this.seconds[value[1]].substring(
                            0,
                            this.seconds[value[1]].length - 1
                        );
                        date = {
                            key:
                                (minute.length == 1 ? '0' + minute : minute) +
                            ...
                            value:
                                this.minutes[value[0]] + this.seconds[value[1]]
                        };
                    }
                    break;
                case 'date':
                    {
                        ...
                        let day = this.days[value[2]].substring(
                            0,
                            this.days[value[2]].length - 1
                        );
                        date = {
                            year: this.years[value[0]].substring(0, 4),
                            month,
                ...
                case 'time':
                    {
                        ...
                        let second = this.seconds[value[2]].substring(
                            0,
                            this.seconds[value[2]].length - 1
                        );
                        date = {
                            hour,
                            minute,
                ...
                case 'yearToMinute':
                    {
                        ...
                        let hour = this.hours[value[3]].substring(
                            0,
                            this.hours[value[3]].length - 1
                        );
                        let minute = this.minutes[value[4]].substring(
                            0,
                            this.minutes[value[4]].length - 1
                        );
                        date = {
                            year: this.years[value[0]].substring(0, 4),
                            month,
                ...
                default:
                    {
                        ...
                        let second = this.seconds[value[5]].substring(
                            0,
                            this.seconds[value[5]].length - 1
                        );
                        date = {
                            year: this.years[value[0]].substring(0, 4),
                            month,
        ...
        getMonths() {
            ...
            if (
                ...
                for (
                    var i = parseInt(this.beginMonth);
                    i <= parseInt(this.endMonth);
                    i++
                ) {
        ...
        getDays() {
            ...
            switch (this.month) {
                ...
                case '12':
                    {
                        if (
                            parseInt(this.beginDay) < 0 ||
                            parseInt(this.beginDay) > 31
                        ) {
                            this.beginDay = 1;
                        }
                        if (
                            parseInt(this.endDay) < 0 ||
                            parseInt(this.endDay) > 31
                        ) {
                            this.endDay = 31;
                        }
                        if (
                            ...
                            for (
                                var i = parseInt(this.beginDay);
                                i <= parseInt(this.endDay);
                                i++
                            ) {
                                this.days.push(i + '日');
                            }
                ...
                case '11':
                    {
                        if (
                            parseInt(this.beginDay) < 0 ||
                            parseInt(this.beginDay) > 30
                        ) {
                            this.beginDay = 1;
                        }
                        if (
                            parseInt(this.endDay) < 0 ||
                            parseInt(this.endDay) > 30
                        ) {
                            this.endDay = 30;
                        }
                        if (
                ...
                case '2':
                    {
                        if (
                            (parseInt(this.year) % 4 == 0 &&
                                parseInt(this.year) % 100 != 0) ||
                            parseInt(this.year) % 400 == 0
                        ) {
                            if (
                                parseInt(this.beginDay) < 0 ||
                                parseInt(this.beginDay) > 29
                            ) {
                                this.beginDay = 1;
                            }
                            if (
                                parseInt(this.endDay) < 0 ||
                                parseInt(this.endDay) > 29
                            ) {
                                this.endDay = 29;
                            }
                            if (
                                parseInt(this.beginYear) ==
                                    parseInt(this.year) &&
                                parseInt(this.endYear) == parseInt(this.year) &&
                                parseInt(this.beginMonth) ==
                                    parseInt(this.month) &&
                            ...
                            if (
                                parseInt(this.beginDay) < 0 ||
                                parseInt(this.beginDay) > 28
                            ) {
                                this.beginDay = 1;
                            }
                            if (
                                parseInt(this.endDay) < 0 ||
                                parseInt(this.endDay) > 28
                            ) {
                                this.endDay = 28;
                            }
                            if (
                ...
                case 'month':
                    {
                        ...
                        this.months.map((item, index) => {
                            if (
                                this.month == item.substring(0, item.length - 1)
                            ) {
                                monthIndex = index;
                            }
                        });
                ...
                case 'day':
                    {
                        ...
                        this.days.map((item, index) => {
                            if (
                                this.day == item.substring(0, item.length - 1)
                            ) {
                                dayIndex = index;
                            }
                        });
                ...
                case 'hour':
                    {
                        ...
                        this.hours.map((item, index) => {
                            if (
                                this.hour == item.substring(0, item.length - 1)
                            ) {
                                hourIndex = index;
                            }
                        });
