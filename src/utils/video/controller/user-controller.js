...
class UserController {
    ...
    userEventHandler(event) {
        ...
        if (event.detail.message && typeof event.detail.message === 'string') {
            try {
                ...
                if (SystemUtils.getSystemInfo().platform !== 'android') {
                    data.userlist.forEach(u => {
                        console.log('user-controller', 'decode user name');
                        try {
                            console.log(decodeURIComponent(escape(u.userid)));
                        } catch (e) {
                            console.error(e);
                        }
                        let playUrl = u.playurl;
                        if (playUrl) {
                            let sp = playUrl.split('?');
                            let params = StringUtils.parse_query_string(sp[1]);
                            params.userid = encodeURI(params.userid);
                            u.playurl = `${sp[0]}?${StringUtils.params(params)}`;
                        }
                    });
                }
                console.log('user-controller', 'userEventHandler', event);
                console.log('user-controller', 'userEventHandler', event.detail.message);
    ...
    _addStream(stream) {
        if (!this.streamList.includes(stream)) {
            console.log('user-controller', '_addStream', stream);
